...
const SearchPresetModal = ({ onClose, onSelect }) => {
  const [page, setPage] = useState(0)
  const size = 10

  const { data, isLoading } = useMyPresets({ page, size, type: 'SEARCH', origin: 'USER' })
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/30'>
      <div className='w-full max-w-4xl rounded-xl bg-white shadow-xl flex flex-col max-h-[80vh]'>
        ...
        <div className='flex items-center justify-between border-b border-gray-200 px-5 py-3'>
          <h2 className='text-base font-semibold text-gray-900'>검색 프리셋 불러오기</h2>
          <button
            type='button'
            onClick={onClose}
...
export default SearchPresetModal
