...
const FieldFilterModal = ({
  ...
  const btnPrimary = 'border-[#2E6BBF] bg-[#2E6BBF] text-white'
      ...
      <div className='w-[720px] max-w-[95vw] rounded-xl bg-white shadow-lg'>
        ...
        <div className='flex items-center justify-between p-4'>
          <div className='text-xl font-semibold'>
            필드: <span className='font-bold'>{fieldName}</span>
          </div>
        ...
        <div className='flex items-center gap-3 px-4'>
          <div className='flex-1'>
            <div className='flex items-center rounded-lg border border-gray-300 px-3'>
              ...
              />
              <ArrowDownIcon className='h-4 w-4 rotate-90 text-gray-400' />
            </div>
          </div>
          <button
            className={`${btn} ${topNEnabled ? btnPrimary : btnMuted}`}
            onClick={() => {
              setTopNOpen((o) => !o)
              ...
              <div className='inline-flex rounded-lg border border-gray-300 bg-white p-1'>
                <button
                  className={`px-3 py-1.5 text-sm rounded-md ${
                    sort === 'asc' ? 'bg-[#2E6BBF] text-white' : 'text-gray-700'
                  }`}
                  onClick={() => setSort('asc')}
                >
                  오름차순
                ...
                <button
                  className={`px-3 py-1.5 text-sm rounded-md ${
                    sort === 'desc' ? 'bg-[#2E6BBF] text-white' : 'text-gray-700'
                  }`}
            ...
            <div className='max-h-[420px] overflow-auto'>
              ...
              {!isLoading &&
                filtered.map((i) => (
                  <label
                    key={i.label}
                    className='flex items-center gap-3 border-t px-3 py-3 text-sm first:border-t-0'
                  >
                    <input
                      type='checkbox'
                      checked={i.checked}
                      onChange={() => toggleOne(i.label)}
                    />
                    <span>{i.label}</span>
                  </label>
                ))}
              {!isLoading && !filtered.length && (
                <div className='py-10 text-center text-xs text-gray-400'>값이 없습니다</div>
              )}
            </div>
          </div>
        </div>
